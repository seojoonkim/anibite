    ...
    rows = db.execute_query(
        ...
        SELECT * FROM (
            ...
            SELECT
                ...
                (SELECT COUNT(*) FROM review_comments WHERE review_id = r.id AND review_type = 'anime') as comments_count
            ...
            SELECT
                ...
                NULL as post_content,
                (SELECT COUNT(*) FROM review_comments WHERE review_id = rev.id AND review_type = 'character') as comments_count
            ...
            SELECT
                ...
                NULL as post_content,
                (SELECT COUNT(*) FROM review_comments WHERE review_id = cr.id AND review_type = 'character') as comments_count
            ...
            SELECT
                ...
                (SELECT COUNT(*) FROM activity_comments WHERE activity_type = 'post' AND activity_user_id = up.user_id AND item_id = up.id) as comments_count
            FROM user_posts up
...
def get_global_feed(limit: int = 50, offset: int = 0) -> List[Dict]:
    """
    전체 사용자의 최근 활동 피드 (UNION ALL로 최적화)

    활동 타입:
    - anime_rating: 애니메이션 평가
    - character_rating: 캐릭터 평가
    - review: 리뷰 작성
    - user_post: 사용자 게시글
    """

    # UNION ALL로 모든 활동을 단일 쿼리로 통합 + DB 레벨에서 정렬/LIMIT
    rows = db.execute_query(
        """
        SELECT * FROM (
            -- 애니메이션 평가 활동
