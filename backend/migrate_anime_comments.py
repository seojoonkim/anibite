"""
애니메이션 평가/리뷰 댓글 통합 마이그레이션 스크립트
activity_comments의 anime_rating 댓글을 review_comments로 이동
"""
from database import Database

def migrate_anime_comments():
    db = Database()

    print("=== 애니메이션 댓글 마이그레이션 시작 ===\n")

    # 1. 현재 상태 확인
    activity_comments_count = db.execute_query(
        "SELECT COUNT(*) FROM activity_comments WHERE activity_type = 'anime_rating'",
        fetch_one=True
    )[0]

    review_comments_count = db.execute_query(
        "SELECT COUNT(*) FROM review_comments WHERE review_type = 'anime'",
        fetch_one=True
    )[0]

    print(f"마이그레이션 전:")
    print(f"  activity_comments (anime_rating): {activity_comments_count}개")
    print(f"  review_comments (anime): {review_comments_count}개\n")

    # 2. activity_comments에서 anime_rating 댓글 가져오기
        ...
        try:
            ...
            if review:
                ...
                rating = db.execute_query(
                    "SELECT rating FROM user_ratings WHERE user_id = ? AND anime_id = ? AND status = 'RATED'",
                    (activity_user_id, anime_id),
                    fetch_one=True
                )
                ...
                db.execute_query("""
                    ...
                    (user_id, anime_id, content, title, is_spoiler, likes_count, created_at, updated_at)
                    VALUES (?, ?, '', NULL, 0, 0, ?, ?)
                """, (activity_user_id, anime_id, created_at, created_at))
            ...
            # 3-3. parent_comment_id 매핑
            new_parent_id = None
            if parent_comment_id:
                parent_mapping = db.execute_query(
                    "SELECT id FROM review_comments WHERE review_type = 'anime' AND review_id = ? AND content = (SELECT content FROM activity_comments WHERE id = ?)",
    ...
    print(f"  review_comments (anime): {new_review_comments_count}개")

    # 5. activity_comments 삭제 확인
    print(f"\n주의: activity_comments의 anime_rating 댓글은 수동으로 삭제해야 합니다.")
    print(f"  확인 후 실행: DELETE FROM activity_comments WHERE activity_type = 'anime_rating'")

if __name__ == "__main__":
    migrate_anime_comments()
