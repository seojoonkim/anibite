...
export default function RateCharacters() {
    ...
    const calculateStarSizes = () => {
      ...
      Object.keys(cardRefs.current).forEach(charId => {
        ...
        if (cardElement) {
          ...
          // Î≥?5Í∞úÍ? Ïπ¥Îìú??ÍΩ?Ï∞®ÎèÑÎ°?Í≥ÑÏÇ∞ (?®Îî© Í≥†Î†§)
          const availableWidth = cardWidth - 12; // Ï¢åÏö∞ ?®Îî© px-2 (8px * 2) Ï§ÑÏûÑ
          const starSize = Math.floor(availableWidth / 5.29); // ???¨Í≤å (4% increase): 5.5 ??5.29
  ...
  const handleRatingChange = async (characterId, rating) => {
    ...
    try {
      ...
      alert(
        language === 'ko'
          ? `?âÍ?Î•??Ä?•Ìïò?îÎç∞ ?§Ìå®?àÏäµ?àÎã§${errorStatus}\n${errorDetail}`
          : language === 'ja'
            ? `Ë©ï‰æ°??øùÂ≠ò„Å´Â§±Êïó?ó„Åæ?ó„Åü${errorStatus}\n${errorDetail}`
  ...
  const handleStatusChange = async (characterId, status) => {
    ...
    try {
      ...
      alert(language === 'ko' ? '?ÅÌÉú Î≥ÄÍ≤ΩÏóê ?§Ìå®?àÏäµ?àÎã§.' : language === 'ja' ? '?π„ÉÜ?º„Çø?πÂ§â?¥„Å´Â§±Êïó?ó„Åæ?ó„Åü?? : 'Failed to change status.');
    }
  };
        ...
        <div className="mb-4 flex justify-center items-center">
          ...
          <div className="flex gap-2 items-center flex-wrap justify-center">
            ...
            <div className="bg-surface px-3 py-1.5 rounded-md shadow-sm hover:shadow-md transition-shadow min-w-[80px] border border-border">
              <div className="text-xs text-text-secondary text-center">{language === 'ko' ? '?âÍ??àÏñ¥?? : language === 'ja' ? 'Ë©ï‰æ°Ê∏à„Åø' : 'Rated'}</div>
              <div className="text-base font-bold text-primary text-center tabular-nums">{(stats.rated || 0).toLocaleString()}</div>
            </div>

            {/* Want to Know */}
            <div className="bg-surface px-3 py-1.5 rounded-md shadow-sm hover:shadow-md transition-shadow min-w-[80px] border border-border">
              <div className="text-xs text-text-secondary text-center">{language === 'ko' ? '?åÍ≥†?∂Ïñ¥?? : language === 'ja' ? '?•„Çä?ü„ÅÑ' : 'Want to Know'}</div>
              <div className="text-base font-bold text-secondary text-center tabular-nums">{(stats.wantToKnow || 0).toLocaleString()}</div>
            </div>

            {/* Not Interested */}
            <div className="bg-surface px-3 py-1.5 rounded-md shadow-sm hover:shadow-md transition-shadow min-w-[80px] border border-border">
              <div className="text-xs text-text-secondary text-center">{language === 'ko' ? 'Í¥Ä?¨ÏóÜ?¥Ïöî' : language === 'ja' ? '?àÂë≥?™„Åó' : 'Not Interested'}</div>
            ...
            <div className="bg-surface px-3 py-1.5 rounded-md shadow-sm hover:shadow-md transition-shadow min-w-[80px] border border-border">
              <div className="text-xs text-text-secondary text-center">{language === 'ko' ? '?âÍ∑† ?âÏ†ê' : language === 'ja' ? 'Âπ≥ÂùáË©ï‰æ°' : 'Avg Rating'}</div>
        ...
        {loading && characters.length === 0 ? (
                        ...
                        {character.role && (
                          <div className={`absolute top-2 left-2 px-2 py-1 rounded text-xs font-bold text-white`} style={{
                            backgroundColor: character.role === 'MAIN'
                              ? '#00E5FF'  // Ï£ºÏó∞: ?åÎ???(?åÎßà??
                              : character.role === 'SUPPORTING'
                                ? '#F59E0B'  // Ï°∞Ïó∞: Ï£ºÌô©??(?åÎ??âÍ≥º ?ÄÎπ?
                                : '#9CA3AF',  // ?ëÏä§?∏Îùº: ?åÏÉâ
                            color: 'white'
                          }}>
                            {character.role === 'MAIN'
                              ? (language === 'ko' ? 'Ï£ºÏó∞' : language === 'ja' ? '‰∏ªÂΩπ' : 'Main')
                              : character.role === 'SUPPORTING'
                                ? (language === 'ko' ? 'Ï°∞Ïó∞' : language === 'ja' ? '?©Êºî' : 'Supporting')
                        ...
                        {(hasRated || characterStatuses[character.id]) && (
                          <div className="absolute top-2 right-2 px-2 py-1 rounded text-xs font-bold text-white shadow-lg" style={{
                            ...
                            {(hasRated || characterStatuses[character.id] === 'RATED')
                              ? (language === 'ko' ? '?âÍ??ÑÎ£å' : language === 'ja' ? 'Ë©ï‰æ°Ê∏à„Åø' : 'Rated')
                              : characterStatuses[character.id] === 'WANT_TO_KNOW'
                                ? (language === 'ko' ? '?åÍ≥†?∂Ïñ¥?? : language === 'ja' ? '?•„Çä?ü„ÅÑ' : 'Want to Know')
                                : (language === 'ko' ? 'Í¥Ä?¨ÏóÜ?¥Ïöî' : language === 'ja' ? '?àÂë≥?™„Åó' : 'Not Interested')}
                          </div>
                        )}
          ...
          <div className="text-center py-12">
            <div className="text-xl text-text-secondary mb-4">
              {language === 'ko' ? '?âÍ????†ÎãàÎ©îÏù¥?òÏù¥ ?ÜÏäµ?àÎã§' : language === 'ja' ? 'Ë©ï‰æ°Ê∏à„Åø??Ç¢?ã„É°?å„ÅÇ?ä„Åæ?õ„Çì' : 'No rated anime'}
            </div>
            <p className="text-text-tertiary">
              {language === 'ko' ? 'Î®ºÏ? ?†ÎãàÎ©îÏù¥?òÏùÑ ?âÍ??òÎ©¥ Ï∫êÎ¶≠?∞Î? ?âÍ??????àÏäµ?àÎã§.' : language === 'ja' ? '?æ„Åö?¢„Éã?°„ÇíË©ï‰æ°?ô„Çã?®„ÄÅ„Ç≠?£„É©??Çø?º„ÇíË©ï‰æ°?ß„Åç?æ„Åô?? : 'Rate anime first to rate characters.'}
            </p>
          </div>
        )}
