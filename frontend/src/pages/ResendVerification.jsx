  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (err.response?.status === 404) {
        setError('?±ë¡?˜ì? ?Šì? ?´ë©”?¼ì…?ˆë‹¤.');
      } else if (err.response?.status === 400) {
        const detail = err.response.data?.detail || '';
        if (detail.includes('already verified')) {
          setError('?´ë? ?¸ì¦??ê³„ì •?…ë‹ˆ?? ë¡œê·¸?¸í•´ì£¼ì„¸??');
        } else {
          setError('?¸ì¦ ?´ë©”???„ì†¡???¤íŒ¨?ˆìŠµ?ˆë‹¤.');
        }
      } else {
        setError('?¸ì¦ ?´ë©”???„ì†¡???¤íŒ¨?ˆìŠµ?ˆë‹¤.');
      }
        ...
        <div className="max-w-md w-full bg-white rounded-2xl shadow-xl p-8">
          <div className="text-center">
            ...
            <p className="text-gray-600 mb-4">
              ...
              ?ˆë¡œ???¸ì¦ ?´ë©”?¼ì„ ë³´ë‚´?œë ¸?µë‹ˆ??
            </p>
            <p className="text-sm text-gray-500">
              ? ì‹œ ??ë¡œê·¸???˜ì´ì§€ë¡??´ë™?©ë‹ˆ??..
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-md w-full bg-white rounded-2xl shadow-xl p-8">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <button
            ...
            onMouseEnter={(e) => !e.target.disabled && (e.target.style.backgroundColor = '#2DD4E4')}
