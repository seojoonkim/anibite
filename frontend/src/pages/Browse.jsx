...
export default function Browse() {
  ...
  const performSearch = async (query) => {
    ...
    try {
      ...
      setError(language === 'ko' ? 'Í≤Ä?âÏóê ?§Ìå®?àÏäµ?àÎã§.' : language === 'ja' ? 'Ê§úÁ¥¢?´Â§±?ó„Åó?æ„Åó?ü„Ä? : 'Search failed.');
      setInitialLoading(false);
    }
  };
  ...
  const loadAnime = async (resetList = false) => {
    try {
      ...
      setError('?†ÎãàÎ©îÏù¥?òÏùÑ Î∂àÎü¨?§Îäî???§Ìå®?àÏäµ?àÎã§.');
      setInitialLoading(false);
      setLoadingMore(false);
    }
  };
  ...
  const getStatusBadge = (status) => {
    const badges = {
      'Currently Airing': { text: 'Î∞©ÏòÅÏ§?, color: 'bg-green-100 text-green-800' },
      'Finished Airing': { text: '?ÑÍ≤∞', color: 'bg-blue-100 text-blue-800' },
      'Not yet aired': { text: 'ÎØ∏Î∞©??, color: 'bg-yellow-100 text-yellow-800' },
    };
    ...
    <div className="min-h-screen pt-10 md:pt-12 bg-transparent">
      ...
      <div className="max-w-[1180px] mx-auto px-4 sm:px-6 lg:px-8 py-6 md:py-8">
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex-1 relative">
              ...
              <input
                ...
                placeholder={language === 'ko' ? '?†ÎãàÎ©îÏù¥?? Ï∫êÎ¶≠??Í≤Ä??..' : language === 'ja' ? '?¢„Éã?°„ÄÅ„Ç≠?£„É©??Çø?ºÊ§úÁ¥?..' : 'Search anime, characters...'}
        ...
        {searchMode && characterList.length > 0 && (
          <div className="mb-6">
            <h2 className="text-lg font-semibold text-gray-800 mb-3">
              {language === 'ko' ? 'Ï∫êÎ¶≠?? : language === 'ja' ? '??É£?©„ÇØ?ø„Éº' : 'Characters'} ({characterList.length})
            </h2>
        ...
        {searchMode && animeList.length > 0 && (
          <h2 className="text-lg font-semibold text-gray-800 mb-3">
            {language === 'ko' ? '?†ÎãàÎ©îÏù¥?? : language === 'ja' ? '?¢„Éã?? : 'Anime'} ({animeList.length})
          </h2>
        )}
                  ...
                  <thead className="bg-gradient-to-r from-gray-50 to-gray-100 border-b-2 border-gray-200">
                    <tr>
                      <th className="px-4 py-3 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider">
                        {language === 'ko' ? '?¨Ïä§?? : language === 'ja' ? '?ù„Çπ?ø„Éº' : 'Poster'}
                      </th>
                      <th className="px-4 py-3 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider">
                        {language === 'ko' ? '?úÎ™©' : language === 'ja' ? '?ø„Ç§?à„É´' : 'Title'}
                      </th>
                      <th className="hidden md:table-cell px-4 py-3 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider">
                        {language === 'ko' ? '?ÅÌÉú' : language === 'ja' ? '?π„ÉÜ?º„Çø?? : 'Status'}
                      </th>
                      <th className="px-4 py-3 text-center text-xs font-semibold text-gray-700 uppercase tracking-wider">
                        {language === 'ko' ? '?∞ÎèÑ' : language === 'ja' ? 'Âπ? : 'Year'}
                      </th>
                      <th className="px-4 py-3 text-center text-xs font-semibold text-gray-700 uppercase tracking-wider">
                        {language === 'ko' ? '?îÏàò' : language === 'ja' ? 'Ë©±Êï∞' : 'Episodes'}
                      </th>
                      <th className="hidden md:table-cell px-4 py-3 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider">
                        {language === 'ko' ? '?•Î•¥' : language === 'ja' ? '?∏„É£?≥„É´' : 'Genres'}
                      </th>
                      <th className="hidden md:table-cell px-4 py-3 text-center text-xs font-semibold text-gray-700 uppercase tracking-wider">
                        {language === 'ko' ? '?âÏ†ê' : language === 'ja' ? 'Ë©ï‰æ°' : 'Rating'}
                      </th>
                    </tr>
                  </thead>
                        ...
                        >
                          ...
                          <td className="px-4 py-2">
                            <Link to={`/anime/${anime.id}`} className="block">
                              <div className="font-semibold text-gray-900 hover:text-[#00E5FF] transition-colors">
                                {titles.primary}
                              </div>
                              {titles.secondary && (
                          ...
                          <td className="hidden md:table-cell px-4 py-3 text-center">
                            {anime.site_rating_count > 0 ? (
                              <div>
                                ...
                                <div className="text-xs text-gray-500 mt-0.5">
                                  {anime.site_rating_count.toLocaleString()}Î™?
                                </div>
                              </div>
                            ) : (
              ...
              <div ref={loadMoreTriggerRef} className="h-20 flex items-center justify-center">
                {loadingMore && (
                  <div className="text-gray-500 text-sm">
                    {language === 'ko' ? 'Î°úÎî© Ï§?..' : language === 'ja' ? 'Ë™?ÅøËæº„Åø‰∏?..' : 'Loading...'}
                  </div>
                )}
                {!initialLoading && !loadingMore && !hasMore && animeList.length > 0 && (
                  <div className="text-gray-400 text-sm">
                    {language === 'ko' ? 'Î™®Îì† ?†ÎãàÎ©îÏù¥?òÏùÑ Î∂àÎü¨?îÏäµ?àÎã§' : language === 'ja' ? '?ô„Åπ?¶„ÅÆ?¢„Éã?°„ÇíË™?ÅøËæº„Åø?æ„Åó?? : 'All anime loaded'}
                  </div>
                )}
              </div>
            )}
