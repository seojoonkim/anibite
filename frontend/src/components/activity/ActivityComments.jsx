  ...
  if (loading) {
    return (
      <div className="mt-4 border-t pt-4">
        <p className="text-[15px] text-gray-500">{language === 'ko' ? '?ìÍ? Î°úÎî© Ï§?..' : language === 'ja' ? '?≥„É°?≥„ÉàË™?ÅøËæº„Åø‰∏?..' : 'Loading comments...'}</p>
      </div>
    );
  }
          ...
          <div className="flex gap-2 items-start">
            ...
            <div className="flex-1 flex gap-2">
              <input
                ...
                placeholder={language === 'ko' ? '?ìÍ????ÖÎ†•?òÏÑ∏??..' : language === 'ja' ? '?≥„É°?≥„Éà?íÂÖ•?õ„Åó?¶„Åè?†„Åï??..' : 'Write a comment...'}
              ...
              <button
                ...
                style={newCommentText.trim() ? { backgroundColor: '#00E5FF' } : {}}
              >
                {language === 'ko' ? '?ëÏÑ±' : language === 'ja' ? '‰ΩúÊàê' : 'Post'}
              </button>
            </div>
          </div>
        </div>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-1.5 mb-0.5">
                      <Link
                        ...
                        className="text-[13px] font-medium text-gray-700 hover:text-[#00E5FF]"
                      >
                    ...
                    <div className="flex items-center gap-2 mt-0.5">
                      {currentUser && (
                        <button
                          ...
                          className="text-[11px] text-gray-500 hover:text-[#00E5FF]"
                        >
                          {language === 'ko' ? '?µÍ?' : language === 'ja' ? 'Ëøî‰ø°' : 'Reply'}
                        </button>
                      )}
                      {currentUser && currentUser.id === comment.user_id && (
                        <button
                          onClick={() => {
                            if (window.confirm(language === 'ko' ? '?ìÍ?????†ú?òÏãúÍ≤†Ïäµ?àÍπå?' : language === 'ja' ? '?≥„É°?≥„Éà?íÂâä?§„Åó?æ„Åô?ãÔºü' : 'Delete this comment?')) {
                    ...
                    {replyingTo === comment.id && (
                      <div className="mt-1.5 flex gap-2">
                        <input
                          ...
                          placeholder={language === 'ko' ? '?µÍ????ÖÎ†•?òÏÑ∏??..' : language === 'ja' ? 'Ëøî‰ø°?íÂÖ•?õ„Åó?¶„Åè?†„Åï??..' : 'Write a reply...'}
                        ...
                        <button
                          ...
                          style={replyText.trim() ? { backgroundColor: '#00E5FF' } : {}}
                        >
                          {language === 'ko' ? '?ëÏÑ±' : language === 'ja' ? '‰ΩúÊàê' : 'Post'}
                        </button>
                        <button
                              ...
                              <div className="flex-1 min-w-0">
                                ...
                                <div className="flex items-center gap-2 mt-0.5">
                                  ...
                                  {currentUser && currentUser.id === reply.user_id && (
                                    <button
                                      onClick={() => {
                                        if (window.confirm(language === 'ko' ? '?µÍ?????†ú?òÏãúÍ≤†Ïäµ?àÍπå?' : language === 'ja' ? 'Ëøî‰ø°?íÂâä?§„Åó?æ„Åô?ãÔºü' : 'Delete this reply?')) {
